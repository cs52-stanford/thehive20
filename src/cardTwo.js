...
const handleSubmit = (props) => {
  ...
  if (!isValid) {
    document.getElementById("Valid College").innerHTML =
  ...
  return (
    <div id="donation-style">
      ...
      <input
        ...
        name="product"
        onChange={(event) => handleChange(props, event)}
        placeholder="Choose not to specify"
        list="productName"
      />
      ...
      >
        <div
          className="next-button previous-color"
          onClick={() => handlePrevious(props)}
        >
          Previous
        </div>
        <div className="next-button" onClick={() => handleSubmit(props)}>
          Submit
        </div>
      </div>
    </div>
